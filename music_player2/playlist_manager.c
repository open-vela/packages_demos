...
static void create_playlist_item(lv_obj_t *parent, int index) {
  ...
  lv_obj_set_style_bg_color(btn, lv_color_hex(0x4A4A4A),
  ...
  lv_obj_add_event_cb(btn, playlist_item_click_cb, LV_EVENT_CLICKED,
...
static void playlist_close_cb(lv_event_t *e) { playlist_manager_close(); }
...
void playlist_manager_create(lv_obj_t *parent) {
  ...
  lv_obj_set_style_bg_color(playlist_container, lv_color_hex(0x000000),
                            LV_PART_MAIN);
  ...
  lv_obj_add_event_cb(playlist_container, playlist_close_cb, LV_EVENT_CLICKED,
                      NULL);
  ...
  int max_items =
      (R.album_count < MAX_PLAYLIST_ITEMS) ? R.album_count : MAX_PLAYLIST_ITEMS;
...
bool playlist_manager_is_open(void) { return playlist_is_open; }
