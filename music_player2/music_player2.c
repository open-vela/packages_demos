...
void app_switch_to_album(int index) {
  if (R.album_count == 0 || index < 0 || index >= R.album_count ||
...
static void app_refresh_album_info(void) {
  if (C.current_album) {
    ...
    const char *display_name =
        (C.current_album->name && strlen(C.current_album->name) > 0)
            ? C.current_album->name
            : "Unknown Song";
    const char *display_artist =
        (C.current_album->artist && strlen(C.current_album->artist) > 0)
...
static void app_refresh_play_status(void) {
  if (C.timers.playback_progress_update == NULL) {
    C.timers.playback_progress_update =
        lv_timer_create(app_playback_progress_update_timer_cb, 1000, NULL);
  }
...
static void app_playback_progress_update_timer_cb(lv_timer_t *timer) {
  ...
  if (position >= 0) {
    ...
    if (abs((int64_t)new_time - (int64_t)C.current_time) >
        100) { // Reduce threshold to 100ms
...
static void app_refresh_volume_countdown_timer(void) {
  if (C.timers.volume_bar_countdown) {
    ...
    C.timers.volume_bar_countdown =
        lv_timer_create(app_volume_bar_countdown_timer_cb, 3000, NULL);
...
static void app_switch_album_event_handler(lv_event_t *e) {
  ...
  switch_album_mode_t direction =
      (switch_album_mode_t)(lv_uintptr_t)lv_event_get_user_data(e);

  if (direction != SWITCH_ALBUM_MODE_PREV &&
      direction != SWITCH_ALBUM_MODE_NEXT) {
...
static bool init_resource(void) {
  ...
  if (R.fonts.size_16.normal == NULL || R.fonts.size_22.bold == NULL ||
      R.fonts.size_24.normal == NULL || R.fonts.size_28.normal == NULL ||
...
static void app_start_updating_date_time(void) {
  ...
  C.timers.refresh_date_time =
      lv_timer_create(app_refresh_date_time_timer_cb, 1000, NULL);
  if (C.timers.refresh_date_time == NULL) {
...
static void read_configs(void) {
  ...
  const char *ssid =
      cJSON_GetStringValue(cJSON_GetObjectItem(wifi_object, "ssid"));
  const char *pswd =
      cJSON_GetStringValue(cJSON_GetObjectItem(wifi_object, "pswd"));
  const int version =
      cJSON_GetNumberValue(cJSON_GetObjectItem(wifi_object, "wpa_ver"));
...
static void reload_music_config(void) {
  ...
  uint32_t bytes_read;
  if (lv_fs_read(&file, buff, file_size, &bytes_read) != LV_FS_RES_OK ||
...
#define MAX_SAFE_ALBUMS 50
  R.album_count =
      (total_albums > MAX_SAFE_ALBUMS) ? MAX_SAFE_ALBUMS : total_albums;
  ...
  for (int i = 0; i < R.album_count; i++) {
    ...
    const char *path =
        cJSON_GetStringValue(cJSON_GetObjectItem(music_object, "path"));
    const char *name =
        cJSON_GetStringValue(cJSON_GetObjectItem(music_object, "name"));
    const char *artist =
        cJSON_GetStringValue(cJSON_GetObjectItem(music_object, "artist"));
    const char *cover =
        cJSON_GetStringValue(cJSON_GetObjectItem(music_object, "cover"));
    const double total_time_double =
        cJSON_GetNumberValue(cJSON_GetObjectItem(music_object, "total_time"));
    const char *color_str =
        cJSON_GetStringValue(cJSON_GetObjectItem(music_object, "color"));
